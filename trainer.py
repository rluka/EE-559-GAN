...
class WGANTrainer():
    ...
    def gen_step(self,
        ...
        # 1) clean the generator gradients
        self.model_gen.zero_grad()

        # 2) compute loss
        loss_gen = -self.model_disc(self.model_gen(z)).mean() # Average over the batch
        loss_gen.backward() # Backpropagate on the loss

        # 3) update the generator weights
    ...
    def disc_step(self,
        ...
        # 1) clear the discriminator gradients
    ...
    def train_epoch(self, train_loader):
        ...
        for batch_num, (real_images, _) in tqdm(enumerate(train_loader)):
            ...
            if batch_num % self.n_disc_steps == 0:
                # this should speed up the training, so that redundant gradients are not computed
