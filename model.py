...
class GeneratorBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding,
        ...
        ### BEGIN SOLUTION
        deconvolution = nn.ConvTranspose2d(in_channels, out_channels, kernel_size, stride, padding, bias=False) # Shold I keep bias=False?
        # weights_init(deconvolution)
        layers.append(deconvolution)
...
class Generator(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        ### BEGIN SOLUTION
        # NOTE: formula for output dimension:
            # X_out = (X_in - 1) * stride - 2 * padding + kernel_size
        kernel_sizes = [4, 4, 4, 4]
        stride_sizes = [2, 2, 2, 2] # stride[0] not influencing the output dims (since the input is 1x1)
